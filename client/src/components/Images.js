  ...
  const translateDoc = async () => {
    ...
    try {
      ...
      if (status === 401) {
        ...
        toast.error(serverMessage || "Invalid image translation request.");
      } else {
        toast.error(serverMessage || "An error occurred while translating the image. Please try again later.");
      }
      setLoading(false);
    }
  };
